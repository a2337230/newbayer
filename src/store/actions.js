...
export const nextSong = ({commit,state},status)=>{
	...
	//假如点击下一首 大于当前章节的课件数
	if(nextIndex>maxCourseLength){//假如是章节的最后一个课件
		//去下一章节  
		if(nowIndex+1<=maxLength){ //没有超过章节数
...
export const prevSong = ({commit,state},status)=>{
	...
	if(nowIndex2==0){
		if(nowIndex!=0){  //不是第一章
			commit(types.SET_ACTIVE_INDEX,nowIndex-1);
			commit(types.SET_ACTIVE_INDEX2,state.playList[nowIndex-1].cw.length-1);
			commit(types.SET_MP3INFO,state.playList[nowIndex-1].cw[state.playList[nowIndex-1].cw.length-1])
		}else{ // 是第一章 就去最后一章的最后一首歌
			commit(types.SET_ACTIVE_INDEX,maxLength-1);
			commit(types.SET_ACTIVE_INDEX2,state.playList[maxLength].cw.length-1);
			commit(types.SET_MP3INFO,state.playList[maxLength].cw[state.playList[maxLength].cw.length-1])
		}
	}else {  //不是当前章节的第一首歌
