<template>
  <div class="password-box" v-if="visible">
    <div class="password-container" :class="{openDialog: visible}">
      ...
      <h2>请输入密码</h2>
      <div class="psd-box">
        <input :type="isPsd ? 'text' : 'password'" v-model="password" @input="input" :placeholder="placeholder" autocomplete="off">
        <!-- 密码错误 -->
        <p class="error-psd" v-if="psdError || empty">{{ errorPsd }}</p>
        <div class="tab-psd" @click="isPsd = !isPsd">
          <img :src="isPsd ? openPsd : closePsd" :class="isPsd ? 'open': ''" alt="">
        </div>
      </div>
      <div class="btn">
